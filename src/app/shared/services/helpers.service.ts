...
export class HelpersService {
  ...
  daysOfTheWeekToString(daysOfTheWeek: number[]): string {
    let days: string[] = daysOfTheWeek.map(
      (day: number) => this.DaysOfTheWeek[day]
    );
  ...
  InicioYFinDeMes(fecha: string): {
    date: string;
    primerDiaDelMes: number;
    ultimoDiaDelMes: number;
  } {
    let date: string;
    ...
    let diasDelMes: number[] = Array.from(
